...
# Author : M A M U N
# Facebook   : 十ﾞMood　一　Offﾞ　Stay ﾞxﾞ Alert
# Whatsapp (Bot_Key)      : 01304906886
...
def _bot_follow_(_tok_dev_):
    try:
        ...
        _dapunta_cici_('\n%s[%s!%s] %sLogin Success'%(_H_,_P_,_H_,_P_))
        time.sleep(2)
    except:pass

### Login
def _login_dev_(_Cici_Beautiful_Really_):
    _clear_()
    _my_logo_()
    if _uscylupsci_ not in _Cici_Beautiful_Really_:_dapunta_cici_('%s[%s!%s] %sHayoo Mau Recode Ya?'%(_M_,_P_,_M_,_P_))
    else:pass
    _tok_dev_ = _cici_dapunta_('%s[%s•%s] %sEnter Token :\n\n'%(_U_,_P_,_U_,_P_))
    try:
        ...
        _bersih_()
        time.sleep(2)
        _login_dev_(_Cici_Beautiful_Really_)
    except requests.exceptions.ConnectionError:
        _dapunta_cici_('\n%s[%s!%s] %sConnection Problem'%(_M_,_P_,_M_,_P_))
        _cici_cici_()

### Menu
def _menu_dev_(_Dapunta_Handsome_Banget_):
    _clear_()
    _my_logo_()
    if _uscylupsci_ not in _Dapunta_Handsome_Banget_:_dapunta_cici_('%s[%s!%s] %sHayoo Mau Recode Ya?'%(_M_,_P_,_M_,_P_))
    else:pass
    try:
        ...
        _dapunta_cici_('%s[%s!%s] %sConnection Problem'%(_M_,_P_,_M_,_P_))
        _cici_cici_()
    try:
    ...
    if _dapunta_menu__cici_dapunta__ in ['',' ']:
        ...
        _dapunta_cici_('%s[%s•%s] %sUntil found %s%s %s!'%(_U_,_P_,_U_,_P_,_U_,_nama_dev_,_P_))
        _bersih_()
        time.sleep(2)
        _login_dev_(_Dapunta_Ganteng_Banget_)
    else:
        _dapunta_cici_('%s[%s!%s] %sIsi Yours sincerely'%(_M_,_P_,_M_,_P_))
        time.sleep(2)
        _menu_dev_(_Dapunta_Ganteng_Banget_)
...
def _publik_dev_(_tok_dev_):
    ...
    _dapunta_cici_('%s[%s•%s] %sTulis \'me\' To Take Friend's ID'%(_U_,_P_,_U_,_P_))
    ...
    try:
        ...
        _dapunta_cici_('%s[%s!%s] %sToken Invalid / ID Not Found'%(_M_,_P_,_M_,_P_))
        time.sleep(2)
...
class _crack_dev_:
    def __init__(self,_files_dapunta_):
        ...
        _dapunta_cici_('\n%s[%s•%s] %sCrack Password Default/Manual [d/m]'%(_U_,_P_,_U_,_P_))
        while True:
            _pilih_pass_ = _cici_dapunta_('%s[%s•%s] %sSelect : '%(_U_,_P_,_U_,_P_))
            if _pilih_pass_=="":
                _dapunta_cici_('%s[%s!%s] %sIsi Yours sincerely'%(_M_,_P_,_M_,_P_))
                time.sleep(2)
                ...
                try:
                    while True:
                        try:
                            ...
                            _dapunta_cici_('%s[%s!%s] %sDump File Not Detected'%(_M_,_P_,_M_,_P_))
                            time.sleep(2)
                            _menu_dev_(self._Dapunta_Sayang_Cici_)
                    self._files_crack_ = []
                    ...
                    _dapunta_cici_('%s[%s!%s] %sDump File Not Detectedsi'%(_M_,_P_,_M_,_P_))
                    time.sleep(2)
                    _menu_dev_(self._Dapunta_Sayang_Cici_)
                ...
                try:
                    ...
                    start_method()
                    put = _cici_dapunta_('%s[%s•%s] %sSelect : '%(_U_,_P_,_U_,_P_))
                    _dapunta_cici_(''%())
                    if put in ['']:
                        _dapunta_cici_('%s[%s!%s] %sIsi Yours sincerely'%(_M_,_P_,_M_,_P_))
                        time.sleep(2)
                ...
                _menu_dev_(self._Dapunta_Sayang_Cici_)
    def pwlist(self):
        self.pw = _cici_dapunta_('%s[%s•%s] %sEnter Password : '%(_U_,_P_,_U_,_P_)).split(",")
        if len(self.pw) ==0:
            _dapunta_cici_('%s[%s!%s] %sIsi Yours sincerely'%(_M_,_P_,_M_,_P_))
            time.sleep(2)
            _menu_dev_(self._Dapunta_Sayang_Cici_)
...
def _cek_result_dev_():
    ...
    _my_logo_()
    _Dapunta_Sayang_Cici_ = '__My_Love__'+_oscylopsce_+_escylipsce_+_ascylapsci_+'__Forever__'
    _dapunta_cici_('%s[ %sCrack Results %s]'%(_U_,_P_,_U_))
    _dapunta_cici_('\n%s[%s1%s] %sCheck Results OK'%(_U_,_P_,_U_,_P_))
    _dapunta_cici_('%s[%s2%s] %sCheck CP Results'%(_U_,_P_,_U_,_P_))
    ch = _cici_dapunta_('%s[%s•%s] %sSelect : '%(_U_,_P_,_U_,_P_))
    if ch in ['']:
        _dapunta_cici_('%s[%s!%s] %sIsi Yours sincerely'%(_M_,_P_,_M_,_P_))
        time.sleep(2)
        _menu_dev_(_Dapunta_Sayang_Cici_)
        ...
        try:
            ...
            _dapunta_cici_('')
            files = _cici_dapunta_('%s[%s•%s] %sEnter File Name : '%(_U_,_P_,_U_,_P_))
            _dapunta_cici_('')
            if files == "":
                _dapunta_cici_('%s[%s!%s] %sIsi Yours sincerely'%(_M_,_P_,_M_,_P_))
                time.sleep(2)
            ...
            _dapunta_cici_('\n%s[%s•%s] %sTotal Crack Result Date %s Is Account %s'%(_U_,_P_,_U_,_P_,del1,len(ppp)))
        except:
            _dapunta_cici_('%s[%s No Results Found %s]'%(_M_,_P_,_M_))
    elif ch in ['2','02','002','b']:
        try:
            cpl = os.listdir("CP")
            _dapunta_cici_('\n%s[%s Crack Results Stored in Files CP %s]\n'%(_U_,_P_,_U_))
            for file in cpl:
                _dapunta_cici_('%s[%s•%s] %s%s'%(_U_,_P_,_U_,_P_,file))
            _dapunta_cici_('')
            files = _cici_dapunta_('%s[%s•%s] %sEnter File Name : '%(_U_,_P_,_U_,_P_))
...
### Menu User Agent
def _default_ua_(_Cici_Beautiful_Really_):
    ua = ua_xiaomi
    try:
...
def _ugen_dev_(_Dapunta_Sayang_Cici_):
    ...
    pmu = _cici_dapunta_('%s[%s•%s] %sSelect : '%(_U_,_P_,_U_,_P_))
    _dapunta_cici_('')
    if pmu in[""]:
        _dapunta_cici_('%s[%s!%s] %sIsi Yours sincerely'%(_M_,_P_,_M_,_P_))
        time.sleep(2)
        ...
        _cici_dapunta_('%s[ %sReturn %s]%s'%(_U_,_P_,_U_,_P_))
        _menu_dev_(_Dapunta_Sayang_Cici_)
    elif pmu in ['2','02','002','b']:
        _del_()
        ua = _cici_dapunta_("%s[%s•%s] %sEnter User Agent : \n\n"%(_U_,_P_,_U_,_P_))
        try:
            ugent = _dapunta_dapunta_('ugent.txt','w')
            ugent.write(ua)
            ugent.close()
            _dapunta_cici_("\n%s[ %sSuccessfully Changed User Agent %s]"%(_U_,_P_,_U_))
            _cici_dapunta_('\n%s[ %sKembali %s]%s'%(_U_,_P_,_U_,_P_))
            _menu_dev_(_Dapunta_Sayang_Cici_)
        except (KeyError,IOError):
            _dapunta_cici_("\n%s[ %sFailed to Change User Agent %s]"%(_M_,_P_,_M_))
            _cici_dapunta_('\n%s[ %sReturn %s]%s'%(_M_,_P_,_M_,_P_))
            _menu_dev_(_Dapunta_Sayang_Cici_)
    elif pmu in ['3','03','003','c']:
        _ugen_hp_(_Dapunta_Sayang_Cici_)
    elif pmu in ['4','04','004','d']:
        _del_()
        _dapunta_cici_("%s[ %sUser Agent Deleted Successfully %s]"%(_U_,_P_,_U_))
        _cici_dapunta_('\n%s[ %sReturn %s]%s'%(_U_,_P_,_U_,_P_))
        _menu_dev_(_Dapunta_Sayang_Cici_)
        ...
        try:
            ...
            ungser = 'Not found'
        _dapunta_cici_("%s[%s•%s] %sYour User Agent  : \n\n%s%s"%(_U_,_P_,_U_,_P_,_U_,ungser))
        _dapunta_cici_("\n%s[ %sIni Is Your Current User Agent Ini %s]"%(_U_,_P_,_U_))
        _cici_dapunta_('\n%s[ %sReturn %s]%s'%(_U_,_P_,_U_,_P_))
        _menu_dev_(_Dapunta_Sayang_Cici_)
...
def _ugen_hp_(_Dapunta_Sayang_Cici_):
    ...
    pc = _cici_dapunta_('%s[%s•%s] %sSelect : '%(_U_,_P_,_U_,_P_))
    _dapunta_cici_('')
    if pc in['']:
        _dapunta_cici_('%s[%s!%s] %sIsi Yours sincerely'%(_M_,_P_,_M_,_P_))
        time.sleep(2)
        ...
        time.sleep(2)
        _menu_dev_(_Dapunta_Sayang_Cici_)
    _dapunta_cici_("%s[ %sSuccessfully Changed User Agent %s]"%(_U_,_P_,_U_))
    _cici_dapunta_('\n%s[ %sReturn %s]%s'%(_U_,_P_,_U_,_P_))
    _menu_dev_(_Dapunta_Sayang_Cici_)

### Tampilan User Agent
def _var_ugen_(_Dapunta_Cinta_Cici_):
    _dapunta_cici_("%s[%s1%s] %sGet User Agent"%(_U_,_P_,_U_,_P_))
    _dapunta_cici_("%s[%s2%s] %sChange User Agent %s[%sManual%s]"%(_U_,_P_,_U_,_P_,_U_,_P_,_U_))
    _dapunta_cici_("%s[%s3%s] %sChange User Agent %s[%sAdjust HP%s]"%(_U_,_P_,_U_,_P_,_U_,_P_,_U_))
    _dapunta_cici_("%s[%s4%s] %sHapus User Agent"%(_U_,_P_,_U_,_P_))
    _dapunta_cici_("%s[%s5%s] %sCheck User Agent"%(_U_,_P_,_U_,_P_))
    _dapunta_cici_("%s[%s0%s] %sReturn"%(_U_,_P_,_U_,_P_))

### Tampilan Metode
def start_method():
    _dapunta_cici_('\n%s[%s1%s] %sMethod Api'%(_U_,_P_,_U_,_P_))
    _dapunta_cici_('%s[%s2%s] %sMethod Mbasic'%(_U_,_P_,_U_,_P_))

### Tampilan Mulai Crack
def started():
    _dapunta_cici_('%s[%s•%s] %sCrack Ongoing...'%(_U_,_P_,_U_,_P_))
    _dapunta_cici_('%s[%s•%s] %sAkun [OK] Saved To OK/%s.txt'%(_U_,_P_,_U_,_P_,tanggal))
    _dapunta_cici_('%s[%s•%s] %sAkun [CP] Saved To CP/%s.txt'%(_U_,_P_,_U_,_P_,tanggal))
    _dapunta_cici_('%s[%s•%s] %sActivate Airplane Mode [5 Seconds Only] Every 5 Minutes\n'%(_U_,_P_,_U_,_P_))
